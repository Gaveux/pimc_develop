  ...
  subroutine numerical_integrator(sys,pimc,pot)
   ...
   ! declare a dynamical array that stores the initial condition
   ...
   do i=2,pimc%NumDiscretisation
       ...
       do j=1,sys%natom
          do k=1,sys%dimen
              OldDiscret(i)%x(k,j) = 2.0*OldDiscret(i-1)%x(k,j) - OldDiscret(i-2)%x(k,j) + 2.0*pimc%invNumDiscretisation**2*sys%invMass(j)*pimc%Beta**2*OldDiscret(i-1)%dVdx(k,j)
          enddo
       enddo
          print *, OldDiscret(i)%x
   enddo 
